...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await Telegram.download(_, message, mystic, file_path):
            ...
            print(f"Song fetched and streamed in {time.time() - start_fetch:.2f} seconds")
        ...
        if await Telegram.download(_, message, mystic, file_path):
            ...
            try:
                ...
                err = e if ex_type == "AssistantErr" else _["general_2"].format(ex_type)
                return await mystic.edit_text(err)
            print(f"Video fetched and streamed in {time.time() - start_fetch:.2f} seconds")
            return await mystic.delete()
        return
    elif url:
        if await YouTube.exists(url):
            ...
            # Add threading for download if not present in SoundCloud.download
            ...
            t.start()
            print(f"SoundCloud fetch & stream started in {time.time() - start_fetch:.2f}s")
            return await mystic.delete()
        else:
            try:
    ...
    if str(playmode) == "Direct":
        ...
        if plist_type:
            ...
            print(f"Playlist reply ready in {time.time() - start_fetch:.2f} seconds")
            ...
            if slider:
                ...
                print(f"Slider reply ready in {time.time() - start_fetch:.2f} seconds")
